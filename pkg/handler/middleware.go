...
func (h *Handler) GetAuth(c *gin.Context) (*string, *vpr.Error) {
	...
	if header == "" {
		return nil, SetError(http.StatusUnauthorized, "empty auth header")
	}

	headerParts := strings.Split(header, " ")

	if len(headerParts) != 2 {
		return nil, SetError(http.StatusUnauthorized, "invalid auth header")
	}

	userID, err := h.services.Authorization.ParseTokenService(headerParts[1])

	if err != nil {
		return nil, SetError(err.Status, err.Message)
	}
